...
import {
  Grid3X3, List, SlidersHorizontal, X, ChevronDown, ChevronUp, Search, Loader2, ChevronLeft, ChevronRight
} from 'lucide-react'
...
export function ProductsPage() {
  ...
  const fetchProducts = useCallback(async () => {
    ...
    if (data) {
      setProducts(prev => useInfiniteScroll && page > 1 ? [...prev, ...data] : data as Product[])
      if (useInfiniteScroll) setHasMore(data.length === pageSize)
    }
    if (count !== null) setTotal(count)
    setLoading(false)
    setLoadingMore(false)
  }, [selectedCategories, priceRange, minRating, onSaleOnly, inStockOnly, searchQ, sortBy, page, categories, useInfiniteScroll])
  ...
  }, [useInfiniteScroll, hasMore, loadingMore])
  ...
  return (
    <div className="min-h-screen bg-background">
      <SEO
        title="All Products"
        description="Browse our complete collection of premium home decor, furniture, lighting, and accessories at Maison Luxe Pakistan."
      />
      {/* Page header */}
      ...
      <div className="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex gap-8">
          ...
          <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2 ml-auto">
                ...
                <button
                  ...
                  className={cn(
                    ...
                    useInfiniteScroll ? 'bg-primary text-primary-foreground border-primary' : 'border-border text-muted-foreground hover:text-foreground'
                  )}
                  title={useInfiniteScroll ? 'Switch to pagination' : 'Switch to infinite scroll'}
                >
                  {useInfiniteScroll ? 'Auto Load' : 'Pages'}
                </button>
              </div>
            </div>
            ...
            {useInfiniteScroll && !hasMore && products.length > 0 && (
              <p className="text-center text-sm text-muted-foreground py-6">All products loaded</p>
            )}
          </div>
        </div>
      </div>
