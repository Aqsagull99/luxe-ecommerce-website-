...
type CartStore = {
  ...
  getTotalSavings: () => { label: string; amount: number }[]
}
  ...
  persist(
    (set, get) => ({
      ...
      getTotal: () => {
        ...
        const volume = get().getVolumeDiscount()
        const shipping = get().getShipping()
        return subtotal - discount - bundle - volume + shipping
      },
      ...
      getBundleDiscount: () => {
        ...
        // 5% off when 3+ of any single product
        const hasBundle = items.some((i) => i.quantity >= 3)
      ...
      getVolumeDiscount: () => {
        // Additional 3% off when 5+ total items
        const totalItems = get().items.reduce((sum, i) => sum + i.quantity, 0)
        if (totalItems < 5) return 0
        const subtotal = get().getSubtotal()
        return subtotal * 0.03
      },

      getTotalSavings: () => {
        const savings: { label: string; amount: number }[] = []
        const bundle = get().getBundleDiscount()
        const volume = get().getVolumeDiscount()
        const coupon = get().getDiscount()
        if (bundle > 0) savings.push({ label: 'Bundle Discount (5%)', amount: bundle })
        if (volume > 0) savings.push({ label: 'Volume Discount (3%)', amount: volume })
        if (coupon > 0) savings.push({ label: 'Coupon Discount', amount: coupon })
        return savings
      },
    }),
    { name: 'maison-cart' }
